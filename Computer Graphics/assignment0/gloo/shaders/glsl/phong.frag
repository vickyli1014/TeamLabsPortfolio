...
vec3 CalcDirectionLight(vec3 normal, vec3 view_dir);
...
void main() {
    ...
    if (directional_light.enabled) {
        frag_color += vec4(CalcDirectionLight(normal, view_dir), 1.0);
    }
}
...
vec3 CalcDirectionLight(vec3 normal, vec3 view_dir) {
    ...
    float direction = 1.0 / (light.direction.x + 
        light.direction.y * distance + 
        light.direction.z * (distance * distance));
