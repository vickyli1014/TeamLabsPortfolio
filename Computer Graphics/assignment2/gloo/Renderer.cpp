...
void Renderer::RecursiveRetrieve(const SceneNode& node, RenderingInfo& info, 
    const glm::mat4& model_matrix) const {
  glm::mat4 local_parent_mat = node.GetTransform().GetLocalToParentMatrix();
  glm::mat4 new_model_matrix = model_matrix * local_parent_mat;
  if (node.IsActive()) {
    auto rendering_comp_ptr = node.GetComponentPtr<RenderingComponent>();
    if (rendering_comp_ptr != nullptr) {
      info.emplace_back(rendering_comp_ptr, new_model_matrix);
    }
  }
