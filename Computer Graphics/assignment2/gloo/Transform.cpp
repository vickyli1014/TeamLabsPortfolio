...
glm::mat4 Transform::GetLocalToAncestorMatrix(SceneNode* ancestor) const {
  ...
  glm::mat4 mat = local_transform_mat_;
  auto parent_pointer = node_.GetParentPtr();
  // stop at ancestor
  while (parent_pointer != ancestor) {
    mat = parent_pointer->GetTransform().GetLocalToParentMatrix() * mat;
    parent_pointer = parent_pointer->GetParentPtr();
  } 
  return mat;
}

glm::mat4 Transform::GetLocalToWorldMatrix() const {
  glm::mat4 local_world_mat_ = GetLocalToAncestorMatrix(nullptr);
  // TODO: calculate local-to-world matrix.
  return local_world_mat_;
}
