...
class Trie:
    ...
    def get_node(self, key, set=False):
        """
        ...
        if type(key) != self.key_type:
            raise TypeError
        node = self
        # loop through all elements in key, to return the child trie of the last key element
        for el in range(len(key)):
            # for values not already in trie, if we don't want to set value raise keyerror
            if key[el:el+1] not in node.children:
                if set:
                    node.children[key[el:el+1]] = Trie(type(key))
                else:
    ...
    def len(self):
        # returns the number of keys with values in the trie
        length = 0
        for _ in self:
            length += 1
        return length
...
def make_word_trie(text):
    ...
    ('you', 1)
    """
    # list of strings of a text separated by puncutation
    sentences = tokenize_sentences(text)
    word_trie = Trie(str)
    # for each word, make a trie of single strings and add to its frequency
...
def make_phrase_trie(text):
    ...
    >>> text = 'hello, hello, how are you? hello, hello, how are you? im doing great and you?'
    >>> t = make_phrase_trie(text)
    >>> for x in t:
    ...     print(x)
    (('hello', 'hello', 'how', 'are', 'you'), 2)
    (('im', 'doing', 'great', 'and', 'you'), 1)
    """
    # list of strings of a text separated by puncutation
    sentences = tokenize_sentences(text)
    phrase_trie = Trie(tuple)
    # for each sentence, make a trie of single tuples (of words) and add to its frequency
    for phrase in sentences:
        # creates a tuple of words, when passed in trie keys are single element tuple of words
...
def autocomplete(trie, prefix, max_count=None):
    ...
    >>> autocomplete(t, "be", 2)
    []
    >>> p = make_word_trie('hello hi hey heyyyy')
    >>> autocomplete(p, 'h', 2)
    ['hello', 'hey']
    >>> autocomplete(p, 'he', 1)
    ['hello']
...
def autocorrect(trie, prefix, max_count=None):
    ...
    >>> autocorrect(t, "bar", 3)
    ['bar', 'bark', 'bat']
    >>> p = make_word_trie('cat chat chair call')
    >>> autocorrect(p, 'ca', 2)
    ['cat', 'call']
    >>> autocorrect(p, 'ch', 2)
    ['chat', 'chair']
    ...
    if max_count > len(words):
        ...
        valid_edits = [ (edit, trie[edit]) for edit in edits if (edit not in words and edit in trie and type(trie[edit]) == int) ]
        # why are incomplete edits included
        # for x in trie:
        #     print(x)
        # print()
        # for v in valid_edits:
        #     print(v)
